 ...
 * It takes input from the environment such as the position of the flappy bird, the tubes etc and responds
 * back with the appropriate action to take.
 * 
 * Author @nellex
 */
...
/**
 * The Q-table forms the heart of the Q-learning algorithm. Maintained for our agent Flappy bird, the table
 * represents the state-action function, i.e. the relationship between a set of states (S) and the set of actions (A) => Q[S,A]. 
 * For a given state 's' and a given action 'a', Q(s,a) denotes the expected reward of doing the action 'a' in the state 's'.
 * 
 * In our learning model, the state of the environment is defined by:
 * (1) speedY: The speed of the flappy bird in the Y-axis, i.e. by what rate the bird is going up or falling down
 * (2) tubeX: The X-coordinate of the next incoming tube, i.e. how far the next tube is from the flappy bird
 * (3) diffY: We define the ideal position from which the flappy bird should pass through to be the very middle of vertical space between the two tubes. The parameter 'diffY' denotes the difference between the Y-coordinate of the flappy bird to the Y-coordinate of our ideal passage position, i.e. how down below or above our flappy bird is from where it should pass from the tube.
 */
var Q_table = {};
 ...
 * (1) Learning rate, alpha: Ranging between [0,1], it determines how quickly should the flappy bird override it's old learned actions with the new ones for the corresponding state
 * (2) Discount factor, gamma: Used for determining the importance of future reward. In our game, if the flappy bird fails to clear the tube, the action which it took recently previously will be penalized more than the action which it took 10 steps ago. This is because it's the recent actions which has a more influence on the success of the bird.
 */
...
function getQ(state, action) {
  ...
  if (!(config in Q_table)) {
     // If there's no entry in the given Q-table for the given state-action pair, return a default reward score as 0
     return 0;
  }
...
/**
 * Function responsible for selecting the appropriate action corresponding to the given state
 * The action which has a higher Q-value for the given state is 'generally' executed 
 * @param {*} state 
 */
function getAction(state) {
  ...
  // Lookup the Q-table for rewards corresponding to Jump and Stay action for the given state
  ...
  if (rewardForStay > rewardForJump) {
    ...
    // This is the case when the reward for both the actions are the same
    // In such a case, we determine randomly the action to be taken
    // Generally, the probability of jumping is lower as compared to stay to mimic the natural scenario
    // We press jump much less occasionally than we let the flappy bird fall
...
/**
 * Function responsible for rewarding the flappy bird according to its performance
 * One thing to note here is that we found the behaviour of our Flappy Bird to be highly episodic.
 * As soon as your flappy bird clears one obstacle, we terminate our episode there and then and reward it postively
 * A new episode is then started for the next obstacle i.e. the next tube which is treated completely independent from the previous one
 * 
 * We reward the flappy bird at the end of an episode, hence we maintain a frame buffer to store the state-action pairs 
 * in a sequential order and decide upon the reward to be awarded for that state-action on the completion of the episode
 * @param {*} reward The amound of reward to be awarded to the Flappy Bird
 * @param {*} wasSuccessful Determines if the reward to be awarded should be negative or positive depending upon if the episode was completed successfully or not
 */
function rewardTheBird(reward, wasSuccessful) {
  ...
  // Iterate over the state-action sequence trail, from the most recent to the most oldest
  for (var i = frameBuffer.length-2; i >= 0 && frameSize > 0; i--) {
    ...
    // Update the Q-value for the state-action pair according to the Q-learning algorithm
    // Ref: https://en.wikipedia.org/wiki/Q-learning
    var futureState = frameBuffer[i+1].env;
    var optimalFutureValue = Math.max(getQ(futureState, actionSet.STAY), getQ(futureState, actionSet.JUMP));
    var updateValue = alpha*(rewardForState + gamma * optimalFutureValue - getQ(state, action));

    setQ(state, action, updateValue)
    frameSize--;
 }
 // Allocating reward is complete, hence clear the frame buffer but still try to maintain the most recent 5 state-action pair
 // Since the last actions taken in the previous episode affects the position of the bird in the next episdoe
...
/**
 * This function is executed for every step in the game and is responsible for forming the state and 
 * delegating the action to be taken back to our flappy bird
 */
function nextStep() {
  ...
  // Else, we'll form our state from the current environment parameters to be ingested by our algorithm
  var state = {
  ...
  if (actionToBeTaken == actionSet.JUMP) {
    birdYSpeed = -1.4;
      ...
      // For stay action, we do nothing but just let the bird go down due to gravity
  }  
}